...
export function isCitationPart(
  part: any,
): part is { type: 'data-citation'; data: CitationPayload } {
  return (
    part &&
    typeof part === 'object' &&
    part.type === 'data-citation' &&
    isCitationData(part.data)
  )
}
...
export function citationsFromMessage(message: any): CitationPayload[] {
  if (!message.parts || !Array.isArray(message.parts)) return []
  return message.parts
    .filter(isCitationPart)
    .map((part: any) => part.data)
    .sort((a: any, b: any) => a.citationIndex - b.citationIndex)
}
