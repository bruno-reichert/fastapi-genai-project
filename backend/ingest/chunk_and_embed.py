...
def ingest_document(
    ...
    *,
    ...
    # Insert and commit in safe, smaller transaction blocks
    BATCH_WRITE_SIZE = 100
    for i in range(0, len(records), BATCH_WRITE_SIZE):
        batch_records = records[i : i + BATCH_WRITE_SIZE]
        batch_vectors = vectors[i : i + BATCH_WRITE_SIZE]

        for record, embedding in zip(batch_records, batch_vectors, strict=True):
        ...
        session.commit()
        print(f"    Committed batch {i // BATCH_WRITE_SIZE + 1} ({len(batch_records)} chunks)...")

    print(f"  Successfully finished {document.accession_number}")
    return len(records)
